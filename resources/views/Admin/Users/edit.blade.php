            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-warning">
                        ...
                        @endif
                        <form role="form" action="{{ route('admin.users.update',$element->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-group row">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" name="name" value="{{ $element->name }}" class="form-control" id="exampleInputEmail1" placeholder="@lang('admin.name')">
                                            </div>
                                            <div class="col-md-6">
                                                <label for="exampleInputEmail1">@lang('admin.email')</label>
                                                <input type="email" name="email" value="{{ $element->email }}" class="form-control" id="exampleInputEmail1" placeholder="@lang('admin.email')">
                                            </div>
                                            <div class="col-md-12">
                                                <label for="exampleInputEmail1">@lang('admin.password')</label>
                                                <input type="password" name="password" class="form-control" id="exampleInputEmail1" placeholder="@lang('admin.password')">
                                            </div>
                                            <div class="col-md-12">
                                                <label for="exampleInputEmail1">@lang('admin.roles')</label>
                                                <select name="role" class="form-control">
                                                    @foreach($roles as $role)
                                                        <option {{ $element->roles()->first()->name == $role->name ? 'selected' : '' }} value="{{ $role->name }}">{{ $role->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
