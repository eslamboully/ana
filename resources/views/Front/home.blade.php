    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on("click", ".kanban-delete", function (e) {
                ...
                addEventListener("click", function () {
                    ...
                    $.ajax({
                        ...
                        'method' : "post",
                        'data' : {count_number: $id,_token: '{{ csrf_token() }}'},
                        fail : () => {
                            alert('something is wrong');
                        }
                    });
            ...
            $(document).on("click",".update-kanban-item",function (e) {
                ...
                let id = $('#edit-item-date').val();

                $.ajax({
                    'url' : "{{ route('very.small.board.update') }}",
                    'method' : "post",
                    'data' : {id: id,title: title,dueDate:dueDate,_token: '{{ csrf_token() }}'},
                    fail : () => {
                        alert('something is wrong');
                    }
                });
            ...
            $(document).on('keypress',".kanban-title-board",function(e) {
                if(e.which == 13) {
                    e.preventDefault();
